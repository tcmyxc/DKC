...
class InceptionV3(nn.Module):
    ...
    def __init__(self, in_channels=3, num_classes=10, mu_threshold=0.5):
        super().__init__()
        self.Conv2d_1a_3x3 = BasicConv2d(in_channels, 32, kernel_size=3, padding=1)
        ...
        # add kgl
        self.kgl = KernelGaussianLayer(in_channels=2048, mu_threshold=mu_threshold)
    ...
    def forward(self, x):
        ...
        # 6 -> 1
        x = self.avgpool(x)
        x = self.dropout(x)  # for mini-imagenet, you should comment this line
        x = x.view(x.size(0), -1)
        x = self.kgl(x)
        x = self.linear(x)
        return x
...
def inceptionv3(in_channels=3, num_classes=10, **kwargs):
    return InceptionV3(in_channels=in_channels, num_classes=num_classes, **kwargs)

if __name__ == "__main__":
    from torchsummary import summary
    model = inceptionv3(mu_threshold=0.2)
    print(model)
