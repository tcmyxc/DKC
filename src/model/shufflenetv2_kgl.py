...
class ShuffleNetV2(nn.Module):
    ...
    def __init__(self, ratio=1, in_channels=3, num_classes=10, mu_threshold=0.5):
        super().__init__()
        if ratio == 0.5:
        ...
        # add kgl
        self.kgl = KernelGaussianLayer(in_channels=out_channels[3], mu_threshold=mu_threshold)
...
def shufflenetv2(in_channels=3, num_classes=10, **kwargs):
    return ShuffleNetV2(in_channels=in_channels, num_classes=num_classes, **kwargs)


if __name__ == "__main__":
    from torchsummary import summary
    model = shufflenetv2(mu_threshold=0.2)
    print(model)
